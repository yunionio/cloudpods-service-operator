...
func (r *VirtualMachine) ValidateCreate() (admission.Warnings, error) {
	...
	if len(r.Spec.VmConfig.RootDisk.Image) == 0 {
		return nil, fmt.Errorf("The RootDisk's 'Image' should not be empty")
	}
	return nil, r.validte()
}
...
func (r *VirtualMachine) ValidateUpdate(old runtime.Object) (admission.Warnings, error) {
	...
	if !networkEqual(vmConfig.Networks, oldVmConfig.Networks) {
		return nil, fmt.Errorf("Can not change 'vmConfig.networks' in Spec")
	}

	// PasswordSpec Check
	if r.Spec.VMPasswordSpec != oldR.Spec.VMPasswordSpec {
		return nil, fmt.Errorf("Can not change Password config in Spec")
	}

	if len(oldR.Spec.Eip) == 0 && len(r.Spec.Eip) > 0 {
		return nil, fmt.Errorf("Can not change Eip when choosing 'Creating new EIP to bind' at the start")
	}
	if oldR.Spec.NewEip == nil && r.Spec.NewEip != nil {
		return nil, fmt.Errorf("Can not change NewEip when choosing 'Binding existing EIP' at the start")
	}
	if oldR.Spec.NewEip != nil && r.Spec.NewEip.ChargeType != r.Spec.NewEip.ChargeType {
		return nil, fmt.Errorf("Can not change 'newEip.eipChargeType")
	}

	if r.Spec.BillDuration != oldR.Spec.BillDuration {
		return nil, fmt.Errorf("Can not change 'billDuration'")
	}
	if r.Spec.AutoRenew != oldR.Spec.AutoRenew {
		return nil, fmt.Errorf("Can not change 'autoRenew'")
	}
